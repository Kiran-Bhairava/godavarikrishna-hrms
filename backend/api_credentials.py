"""
Credential management utilities and endpoints.
...
Integration:
- Import these functions into your router modules
- Use the schemas from schemas.py for request/response validation
"""
...
async def generate_credentials_for_employee(
    ...
    """
    Generate new credentials for employee.
    ...
    Args:
        emp_id: Employee ID
        notes: Optional notes about credential generation
        hr_id: User ID of HR person generating credentials
        db: Database connection
    
    Returns:
        Dict with employee info and temporary password
    """
    ...
    async with db.transaction():
        ...
        await db.execute(
            ...
            "generated",
            hr_id,
            notes or f"Generated by HR user {hr_id}",
        )
...
async def get_credential_audit(
    ...
    """
    View credential history for employee.
    
    Shows when password was generated, reset, changed, etc.
    """
...
async def change_password_user_initiated(
    ...
    if not pwd_context.verify(old_password, current_user["password_hash"]):
        ...
        await db.execute(
            ...
            user_id, user_email, "invalid_password_on_change",
        )
...
async def reset_password_by_admin(
    ...
    Used for password recovery or forced resets.
    """
    # Verify user exists
    ...
    logger.info("Password reset: user_id=%s by hr_id=%s", user_id, hr_id)
    return {
        "user_id": user_id,
